...
/*   Updated: 2022/12/12 15:33:43 by raruiz-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (mlx_clear_window(map->mlx, map->win) == false)
		exit(0);
	if (flag == 1)
	{
		mlx_string_put(map->mlx, map->win, 62, 80, 0xFFFAFAFA, "Game over\n");
		ft_printf("Better luck next time, bud.\n");
		map->is_dead = true;
		return (0);
	}
	else
...
int	main(int argc, char **argv)
{
	...
	if (argc == 2)
	{
		...
		mlx_loop_hook(map.mlx, enemy_moves, &map);
		mlx_loop(map.mlx);
	}
	else
